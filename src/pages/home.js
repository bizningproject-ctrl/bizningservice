      ...
      <div class="hero-content">
        ...
        <div class="hero-visual">
          <div class="hero-tools-grid">
            <div class="hero-tool" data-tool="wrench" data-service="plumbing">
              ...
              <span class="hero-tool-label">${t('svc_plumbing')}</span>
            </div>
            <div class="hero-tool" data-tool="bolt" data-service="electrical">
              ...
              <span class="hero-tool-label">${t('svc_electrical')}</span>
            </div>
            <div class="hero-tool" data-tool="sparkle" data-service="cleaning">
              ...
              <span class="hero-tool-label">${t('svc_cleaning')}</span>
            </div>
            <div class="hero-tool" data-tool="roller" data-service="painting">
              ...
              <span class="hero-tool-label">${t('svc_painting')}</span>
            </div>
            <div class="hero-tool" data-tool="snowflake" data-service="hvac">
              ...
              <span class="hero-tool-label">${t('svc_hvac')}</span>
            </div>
            <div class="hero-tool" data-tool="hammer" data-service="handyman">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 12l-8.5 8.5c-.83.83-2.17.83-3 0 0 0 0 0 0 0a2.12 2.12 0 0 1 0-3L12 9"/><path d="M17.64 15L22 10.64"/><path d="M20.91 11.7l-1.25-1.25c-.6-.6-.93-1.4-.93-2.25v-.86L16.01 4.6a5.56 5.56 0 0 0-3.94-1.64H9l.92.82A6.18 6.18 0 0 1 12 8.4v1.56l2 2h2.47l2.26 1.91"/></svg>
              <span class="hero-tool-label">${t('svc_handyman')}</span>
            </div>
          </div>
        </div>
      </div>
...
function initHomeInteractions(container) {
  ...
  // Wave ripple effect on tool icons
  const tools = container.querySelectorAll('.hero-tool');
  const cols = 3;
  ...
  tools.forEach((tool, idx) => {
    ...
    tool.addEventListener('click', () => {
      const type = tool.dataset.service;
      if (type) navigate(`/service?type=${type}`);
    });
    ...
    tool.addEventListener('mouseenter', () => {
      tools.forEach((other, j) => {
        if (j === idx) return;
        const oRow = Math.floor(j / cols);
        const oCol = j % cols;
        const dx = oCol - col;
        const dy = oRow - row;
        const dist = Math.sqrt(dx * dx + dy * dy);
        // Push away: closer = stronger, direction = away from hovered
        const strength = 12 / dist;
        const tx = (dx / dist) * strength;
        const ty = (dy / dist) * strength;
        other.style.translate = `${tx}px ${ty}px`;
      });
    });

    tool.addEventListener('mouseleave', () => {
      tools.forEach((other) => {
        other.style.translate = '';
      });
    });
  });
}
